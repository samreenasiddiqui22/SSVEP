...
async def eeg_explorer():
    """connects to eeg data stream
    """

    def match_eeg_uuid(device: BLEDevice, adv: AdvertisementData):
        # This assumes that the device includes the EEG service UUID in the
        # advertising data. This test may need to be adjusted depending on the
        # actual advertising data supplied by the device.
        if EEG_SERVICE_UUID.lower() in adv.service_uuids:
            return True

        return False

    device = await BleakScanner.find_device_by_filter(match_eeg_uuid)
    print(device)
    if device is None:
        print("no matching device found, you may need to edit match_eeg_uuid().")
        sys.exit(1)
    ...
    async with BleakClient(device, disconnected_callback=handle_disconnect) as client:
        await client.start_notify(EEG_TX_CHAR_UUID, handle_rx)

        print("Connected, start typing and press ENTER...")

        loop = asyncio.get_running_loop()
        eeg = client.services.get_service(EEG_SERVICE_UUID)
        rx_char = eeg.get_characteristic(EEG_RX_CHAR_UUID)
